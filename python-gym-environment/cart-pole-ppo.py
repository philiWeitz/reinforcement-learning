...
from tensorflow.keras.layers import Dense, Input, TimeDistributed, Flatten, LSTM
...
class PPOAgent:
    def __init__(self, state_size, action_size, gamma=GAMMA, lr_actor=LR_ACTOR, lr_critic=LR_CRITIC, load_model_from_file=False):
        ...
        if self.load_model_from_file:
            self.actor.load_weights('logs/cart_pole_ppo_actor.h5')
            self.critic.load_weights('logs/cart_pole_ppo_critic.h5')
    ...
    def save_models(self):
        self.actor.save("logs/cart_pole_ppo_actor.h5")
        self.critic.save("logs/cart_pole_ppo_critic.h5")
...
for r in itertools.product(lrs_actor, lrs_critic):
    ...
    train_agent(nr_of_episodes=500, lr_actor=lr_actor, lr_critic=lr_critic)
